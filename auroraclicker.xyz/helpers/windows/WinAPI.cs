...
using static helpers.NTStatus;

namespace helpers {
    class WinAPI {
        [DllImport( "user32.dll" , CharSet = CharSet.Auto )] public static extern IntPtr SendMessage( IntPtr hWnd , uint msg , int wParam , int lParam );
        [DllImport( "kernel32.dll" )] public static extern int VirtualQueryEx( IntPtr hProcess , IntPtr lpAddress , out WinStructs.MEMORY_BASIC_INFORMATION lpBuffer , uint dwLength );
        [DllImport( "ntdll.dll" , SetLastError = true )] public static extern NtStatus NtWriteVirtualMemory( IntPtr ProcessHandle , IntPtr BaseAddress , byte [ ] Buffer , int NumberOfBytesToWrite , out int NumberOfBytesWritten );
        [DllImport( "ntdll.dll" , SetLastError = true )]  public static extern NtStatus NtReadVirtualMemory( IntPtr ProcessHandle , IntPtr BaseAddress , byte [ ] Buffer , int NumberOfBytesToRead , out int NumberOfBytesRead );
        [DllImport( "kernel32.dll" )] public static extern IntPtr OpenProcess( uint processAccess , bool bInheritHandle , uint processId );
        ...
        [DllImport( "kernel32.dll" )] public static extern void GetSystemInfo( out WinStructs.SystemInfo Info );
    }
}
